...
import 'package:deep_injectable_generator/code_builder/builder_utils.dart';
...
import 'package:deep_injectable_generator/models/external_module_config.dart';
import 'package:deep_injectable_generator/models/injected_dependency.dart';
import 'package:deep_injectable_generator/models/module_config.dart';
import 'package:deep_injectable_generator/utils.dart';

import '../injectable_types.dart';

const _injectableImport = 'package:deep_injectable/deep_injectable.dart';
...
mixin SharedGeneratorCode {
  ...
  Expression _buildParamAssignment(
    ...
    final expression = refer(isAsync ? getAsyncReferName : getReferName).call([], {
      ...
      if (iDep.isParamReceiver) 'param1': refer(iDep.paramName),
    }, [
...
class LibraryGenerator with SharedGeneratorCode {
  ...
  Library generate() {
    ...
    var scopedDeps = groupBy<DependencyConfig, String?>(dependencies, (d) => d.scope);
...
class InitMethodGenerator with SharedGeneratorCode {
        ...
        ..body = Block(
          (b) => b.statements.addAll([
            if (scopeName != null)
              ...
              if (!isMicroPackage)
                if (dependencies.isNotEmpty || microPackagesModulesAfter.isNotEmpty || microPackagesModulesBefore.isNotEmpty)
              ...
              if (usesConstructorCallback)
                declareFinal('ccb')
                    .assign(
                      refer('constructorCallback').ifNullThen(CodeExpression(Code('<T>(_) => _'))),
                    )
                    .statement,
              ...ghStatements,
  ...
  Code buildLazyRegisterFun(DependencyConfig dep) {
    ...
    if (dep.injectableType == InjectableType.factory) {
      final hasFactoryParams = dep.dependencies.any((d) => d.isFactoryParam || d.isParamReceiver);
      if (hasFactoryParams) {
  ...
  Map<String, Reference> _resolveFactoryParams(DependencyConfig dep) {
    ...
    if (params.isEmpty) {
      if(dep.dependencies.any((d) => d.isParamReceiver)){
        params['passedParam'] = refer('dynamic');
      }
    }
    if (params.length < 2) {
